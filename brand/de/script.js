/* ===========================================================
   Paper Core — Brand ad page · interactions (WARM · DE)
...
(function () {
  ...
  var DEMOS = [
    {
      title: "Aufmerksamkeitsgesteuerte spärliche<br>Transformer für die Langdokument-Suche",
      auth: "M. Hoffmann, L. Schäfer, T. Weber · 2025",
      paperFx: "A(Q,K,V)=softmax(QKᵀ⁄√d<sub>k</sub>)·M&thinsp;V",
      summary: "Dieses Paper stellt einen spärlichen Aufmerksamkeitsmechanismus vor, der die Langdokument-Suche auf O(n log n) skaliert, die Abfragelatenz um das 4,1-Fache senkt und die Trefferquote dabei innerhalb von 0,6 Punkten dichter Baselines hält.",
      keywords: ["spärliche Aufmerksamkeit", "Langdokument-Suche", "O(n log n)", "4,1× schneller", "Recall ≈ dicht", "Transformer"],
    ...
    {
      title: "Kontrastives Vortraining für<br>Proteinfaltung bei wenig Daten",
      auth: "D. Fischer, A. Yilmaz · 2024",
      paperFx: "ℒ = −log( exp(s<sup>+</sup>⁄τ) ⁄ Σ exp(s<sup>i</sup>⁄τ) )",
      summary: "Ein kontrastives Ziel trainiert das Faltungsmodell auf unbeschrifteten Sequenzen vor, hebt die Genauigkeit bei datenarmen Familien um 12,7 GDT-TS und erreicht überwachte Baselines mit 8× weniger beschrifteten Daten.",
      keywords: ["kontrastives Lernen", "Proteinfaltung", "wenig Daten", "+12,7 GDT-TS", "8× weniger Daten", "selbstüberwacht"],
    ...
    {
      title: "Diffusions-Priors für<br>inverse Bildgebungsprobleme",
      auth: "K. Brandt, J. Wagner, P. Novák · 2025",
      paperFx: "x<sub>t</sub> = √α̅<sub>t</sub>·x<sub>0</sub> + √(1−α̅<sub>t</sub>)·ε",
      summary: "Ein vortrainierter Diffusions-Prior in der Rekonstruktionsschleife stellt beschädigte Scans mit 3,2 dB höherem PSNR wieder her als Total-Variation-Methoden — ganz ohne aufgabenspezifisches Nachtrainieren.",
      keywords: ["Diffusions-Prior", "inverse Probleme", "Bildrestaurierung", "+3,2 dB PSNR", "kein Nachtraining", "bayessch"],
  ...
  function runOne(idx) {
    ...
    setDots(idx);
    swapPaper(d);
    if (dstatus) { dstatus.textContent = "● Analyse läuft"; dstatus.classList.remove("done"); }
    chipsBox.innerHTML = "";
    ...
    demoTimer = setTimeout(function tick() {
      if (i <= d.summary.length) {
        ...
        setTimeout(function () { if (dstatus) { dstatus.textContent = "● fertig"; dstatus.classList.add("done"); } }, tail + 200);
  ...
  function runDemoStatic() {
    ...
    revealOut(); setDots(0);
    if (dstatus) { dstatus.textContent = "● fertig"; dstatus.classList.add("done"); }
  }
