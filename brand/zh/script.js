...
(function () {
  ...
  var DEMOS = [
    {
      title: "面向长文档检索的<br>注意力引导稀疏 Transformer",
      auth: "李伟、王芳、陈昊 · 2025",
      paperFx: "A(Q,K,V)=softmax(QKᵀ⁄√d<sub>k</sub>)·M&thinsp;V",
      summary: "本文提出一种稀疏注意力机制，将长文档检索扩展到 O(n log n)，在召回率较稠密基线相差 0.6 个点以内的同时，把查询延迟降低 4.1 倍。",
      keywords: ["稀疏注意力", "长文档检索", "O(n log n)", "4.1倍加速", "召回≈稠密", "Transformer"],
    ...
    {
      title: "低资源蛋白质结构预测的<br>对比预训练",
      auth: "刘洋、赵敏 · 2024",
      paperFx: "ℒ = −log( exp(s<sup>+</sup>⁄τ) ⁄ Σ exp(s<sup>i</sup>⁄τ) )",
      summary: "对比学习目标在无标注序列上预训练折叠模型，将低资源家族的精度提升 12.7 GDT-TS，并以 1/8 的标注数据匹敌有监督基线。",
      keywords: ["对比学习", "蛋白质结构预测", "低资源", "+12.7 GDT-TS", "数据1/8", "自监督"],
    ...
    {
      title: "逆成像问题的<br>扩散先验",
      auth: "孙琳、周宇 · 2025",
      paperFx: "x<sub>t</sub> = √α̅<sub>t</sub>·x<sub>0</sub> + √(1−α̅<sub>t</sub>)·ε",
      summary: "将预训练的扩散先验接入重建回路，无需针对任务重新训练，即可以比全变分方法高 3.2 dB 的 PSNR 复原受损的扫描图像。",
      keywords: ["扩散先验", "逆问题", "图像复原", "+3.2 dB PSNR", "零重训", "贝叶斯"],
  ...
  function runOne(idx) {
    ...
    demoTimer = setTimeout(function tick() {
      if (i <= d.summary.length) {
        ...
        setTimeout(function () { if (dstatus) { dstatus.textContent = "● 完成"; dstatus.classList.add("done"); } }, tail + 200);
  ...
  function runDemoStatic() {
    ...
    revealOut(); setDots(0);
    if (dstatus) { dstatus.textContent = "● 完成"; dstatus.classList.add("done"); }
  }
