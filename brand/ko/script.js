/* ===========================================================
   Paper Core — Brand ad page · interactions (WARM · KO)
...
(function () {
  ...
  var DEMOS = [
    {
      title: "장문 검색을 위한<br>어텐션 유도 희소 트랜스포머",
      auth: "김민준, 이서연, 박지호 · 2025",
      paperFx: "A(Q,K,V)=softmax(QKᵀ⁄√d<sub>k</sub>)·M&thinsp;V",
      summary: "이 논문은 장문 검색을 O(n log n)으로 확장하는 희소 어텐션 메커니즘을 제안합니다. 질의 지연을 4.1배 줄이면서 재현율은 밀집 기준선의 0.6포인트 이내로 유지합니다.",
      keywords: ["희소 어텐션", "장문 검색", "O(n log n)", "4.1배 빠름", "재현율 ≈ 밀집", "트랜스포머"],
    ...
    {
      title: "저자원 단백질 폴딩을 위한<br>대조 사전학습",
      auth: "정하은, 오준서 · 2024",
      paperFx: "ℒ = −log( exp(s<sup>+</sup>⁄τ) ⁄ Σ exp(s<sup>i</sup>⁄τ) )",
      summary: "대조 학습 목표로 라벨 없는 서열에서 폴딩 모델을 사전학습해, 저자원 계열의 정확도를 12.7 GDT-TS 높이고 라벨 데이터를 8배 적게 쓰고도 지도학습 기준선과 동등한 성능을 냅니다.",
      keywords: ["대조 학습", "단백질 폴딩", "저자원", "+12.7 GDT-TS", "데이터 8배 절감", "자기지도"],
    ...
    {
      title: "역영상 문제를 위한<br>확산 사전확률(prior)",
      auth: "한도윤, 임예린, 강태우 · 2025",
      paperFx: "x<sub>t</sub> = √α̅<sub>t</sub>·x<sub>0</sub> + √(1−α̅<sub>t</sub>)·ε",
      summary: "사전학습된 확산 prior를 복원 루프에 결합하면, 작업별 재학습 없이도 손상된 스캔을 전변분(total-variation) 기법보다 3.2 dB 높은 PSNR로 복원합니다.",
      keywords: ["확산 prior", "역문제", "영상 복원", "+3.2 dB PSNR", "재학습 불필요", "베이지안"],
  ...
  function runOne(idx) {
    ...
    setDots(idx);
    swapPaper(d);
    if (dstatus) { dstatus.textContent = "● 분석 중"; dstatus.classList.remove("done"); }
    chipsBox.innerHTML = "";
    ...
    demoTimer = setTimeout(function tick() {
      if (i <= d.summary.length) {
        ...
        setTimeout(function () { if (dstatus) { dstatus.textContent = "● 완료"; dstatus.classList.add("done"); } }, tail + 200);
  ...
  function runDemoStatic() {
    ...
    revealOut(); setDots(0);
    if (dstatus) { dstatus.textContent = "● 완료"; dstatus.classList.add("done"); }
  }
